    ...
    while (true) {
        ...
        switch (estilo_borda) {
            ...
            case 2:
                ...
                break;
            case 3:
                // Estilo 4: borda tracejada (tamanho igual aos outros estilos)
                for (int i = 0; i < LARGURA_DISPLAY; i += 4) {
                    ssd1306_rect(&display, i + 3, 0, 2, 2, true, false);  // Linha superior
                    ssd1306_rect(&display, i + 3, ALTURA_DISPLAY - 2, 2, 2, true, false);  // Linha inferior
                }
                for (int i = 0; i < ALTURA_DISPLAY; i += 4) {
                    ssd1306_rect(&display, 0, i + 3, 2, 2, true, false);  // Linha esquerda
                    ssd1306_rect(&display, LARGURA_DISPLAY - 2, i + 3, 2, 2, true, false);  // Linha direita
                }
                break;
        }
